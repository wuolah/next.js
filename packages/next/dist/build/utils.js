...
const _config = require("../server/config");
...
async function printTreeView(lists, pageInfos, { distPath , buildId , pagesDir , pageExtensions , buildManifest , appBuildManifest , middlewareManifest , useStatic404 , gzipSize =true  }) {
...
async function isPageStatic({ page , distDir , configFileName , runtimeEnvConfig , httpAgentOptions , locales , defaultLocale , parentId , pageRuntime , edgeInfo , pageType , hasServerComponents , originalAppPath , isrFlushToDisk , maxMemoryCacheSize , incrementalCacheHandlerPath  }) {
    ...
    return isPageStaticSpan.traceAsyncFn(async ()=>{
        ...
        if (pageType === "app") {
            var _componentsResult_ComponentMod_routeModule;
            ...
            const userland = (_componentsResult_ComponentMod_routeModule = componentsResult.ComponentMod.routeModule) == null ? void 0 : _componentsResult_ComponentMod_routeModule.userland;
...
async function getNamedExports(page, distDir, runtimeEnvConfig) {
    require("../shared/lib/runtime-config").setConfig(runtimeEnvConfig);
    const components = await (0, _loadcomponents.loadComponents)({
        distDir,
        pathname: page,
...
async function copyTracedFiles(dir, distDir, pageKeys, appPageKeys, tracingRoot, serverConfig, middlewareManifest, hasInstrumentationHook) {
    ...
    await _fs.promises.writeFile(serverOutputPath, `${moduleType ? `import http from 'http'
