...
async function build(dir, reactProductionProfiling = false, debugOutput = false, runLint = true, noMangling = false, appDirOnly = false, turboNextBuild = false, buildMode) {
    ...
    try {
        ...
        const buildResult = await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            if (!isGenerate && config.outputFileTracing) {
                ...
                await nextBuildSpan.traceChild("trace-next-server").traceAsyncFn(async ()=>{
                    ...
                    if (incrementalCacheHandlerPath) {
                        toTrace.push(require.resolve(_path.default.isAbsolute(incrementalCacheHandlerPath) ? incrementalCacheHandlerPath : _path.default.join(dir, incrementalCacheHandlerPath)));
                    }
                    let serverResult;
                    ...
                    function addToTracedFiles(base, file) {
                        tracedFiles.add(_path.default.relative(distDir, _path.default.join(base, file)).replace(/\\/g, "/"));
                    }
                    if (isStandalone) {
            ...
            if (ssgPages.size > 0 || appDir) {
                ...
                await _fs.promises.writeFile(_path.default.join(distDir, _constants1.PRERENDER_MANIFEST).replace(/\.json$/, ".js"), `self.__PRERENDER_MANIFEST=${JSON.stringify(prerenderManifest)}`, "utf8");
