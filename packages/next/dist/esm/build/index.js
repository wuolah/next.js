...
import { BUILD_ID_FILE, BUILD_MANIFEST, CLIENT_STATIC_FILES_PATH, EXPORT_DETAIL, EXPORT_MARKER, FONT_MANIFEST, IMAGES_MANIFEST, PAGES_MANIFEST, PHASE_PRODUCTION_BUILD, PRERENDER_MANIFEST, CLIENT_REFERENCE_MANIFEST, REACT_LOADABLE_MANIFEST, ROUTES_MANIFEST, SERVER_DIRECTORY, SERVER_FILES_MANIFEST, STATIC_STATUS_PAGES, MIDDLEWARE_MANIFEST, APP_PATHS_MANIFEST, APP_PATH_ROUTES_MANIFEST, APP_BUILD_MANIFEST, FLIGHT_SERVER_CSS_MANIFEST, RSC_MODULE_TYPES, NEXT_FONT_MANIFEST, SUBRESOURCE_INTEGRITY_MANIFEST, MIDDLEWARE_BUILD_MANIFEST, MIDDLEWARE_REACT_LOADABLE_MANIFEST, TURBO_TRACE_DEFAULT_MEMORY_LIMIT, TRACE_OUTPUT_VERSION, SERVER_REFERENCE_MANIFEST } from "../shared/lib/constants";
...
async function generateClientSsgManifest(prerenderManifest, { buildId , distDir , locales  }) {
    ...
    await promises.writeFile(path.join(distDir, CLIENT_STATIC_FILES_PATH, buildId, "_ssgManifest.js"), clientSsgManifestContent);
}
...
export default async function build(dir, reactProductionProfiling = false, debugOutput = false, runLint = true, noMangling = false, appDirOnly = false, turboNextBuild = false, buildMode) {
    const isCompile = buildMode === "experimental-compile";
    const isGenerate = buildMode === "experimental-generate";
    let hasAppDir = false;
    try {
        const nextBuildSpan = trace("next-build", undefined, {
            version: "13.4.4-canary.2"
        });
        ...
        const buildResult = await nextBuildSpan.traceAsyncFn(async ()=>{
            ...
            let buildId = "";
            if (isGenerate) {
                buildId = await promises.readFile(path.join(distDir, "BUILD_ID"), "utf8");
            } else {
            ...
            function createStaticWorker(type) {
                const numWorkersPerType = isAppDirEnabled ? Math.max(1, ~~(numWorkers / 2)) : numWorkers;
            ...
            const { customAppGetInitialProps , namedExports , isNextImageImported , hasSsrAmpPages , hasNonStaticErrorPage  } = await staticCheckSpan.traceAsyncFn(async ()=>{
                ...
                const namedExportsPromise = pagesStaticWorkers.getNamedExports(appPageToCheck, distDir, runtimeEnvConfig);
            ...
            if (!isGenerate && config.outputFileTracing) {
                ...
                await nextBuildSpan.traceChild("trace-next-server").traceAsyncFn(async ()=>{
                    ...
                    if (incrementalCacheHandlerPath) {
                        toTrace.push(require.resolve(path.isAbsolute(incrementalCacheHandlerPath) ? incrementalCacheHandlerPath : path.join(dir, incrementalCacheHandlerPath)));
                    }
                    let serverResult;
                    ...
                    function addToTracedFiles(base, file) {
                        tracedFiles.add(path.relative(distDir, path.join(base, file)).replace(/\\/g, "/"));
                    }
                    if (isStandalone) {
                        addToTracedFiles("", require.resolve("next/dist/compiled/jest-worker/processChild"));
                        addToTracedFiles("", require.resolve("next/dist/compiled/jest-worker/threadChild"));
                    }
                    if (config.experimental.turbotrace) {
            ...
            const middlewareManifest = JSON.parse(await promises.readFile(path.join(distDir, SERVER_DIRECTORY, MIDDLEWARE_MANIFEST), "utf8"));
                    ...
                    async function moveExportedAppNotFoundTo404() {
                        return staticGenerationSpan.traceChild("move-exported-app-not-found-").traceAsyncFn(async ()=>{
                            ...
                            if (await fileExists(orig)) {
                                await promises.copyFile(orig, path.join(distDir, "server", updatedRelativeDest));
            ...
            if (ssgPages.size > 0 || appDir) {
                ...
                await promises.writeFile(path.join(distDir, PRERENDER_MANIFEST).replace(/\.json$/, ".js"), `self.__PRERENDER_MANIFEST=${JSON.stringify(prerenderManifest)}`, "utf8");
