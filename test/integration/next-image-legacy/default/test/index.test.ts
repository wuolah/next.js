  ...
  it('should initially load only two of four images using lazyroot', async () => {
    ...
    try {
      ...
      expect(
        await hasImageMatchingUrl(
          browser,
          `http://127.0.0.1:${appPort}/_next/image?url=%2Ftest.png&w=828&q=75`
        )
      ).toBe(true)
      expect(
        await hasImageMatchingUrl(
          browser,
          `http://127.0.0.1:${appPort}/test.svg`
        )
      ).toBe(true)
      expect(
        await hasImageMatchingUrl(
          browser,
          `http://127.0.0.1:${appPort}/_next/image?url=%2Ftest.webp&w=828&q=75`
        )
      ).toBe(false)
    } finally {
      if (browser) {
