...
it('allows to the pathname', async () => {
  ...
  expect(parsed.hostname).toEqual('127.0.0.1')
  expect(parsed.host).toEqual('127.0.0.1:3000')
  expect(parsed.href).toEqual('http://127.0.0.1:3000/about?param1=value1')

  parsed.pathname = '/hihi'
  expect(parsed.href).toEqual('http://127.0.0.1:3000/hihi?param1=value1')
})

it('allows to change the host', () => {
  const parsed = new NextURL('/about?param1=value1', 'http://127.0.0.1')
  expect(parsed.hostname).toEqual('127.0.0.1')
  expect(parsed.host).toEqual('127.0.0.1')
  expect(parsed.href).toEqual('http://127.0.0.1/about?param1=value1')
...
it('allows to change the whole href', () => {
  const url = new NextURL('https://127.0.0.1.com/foo')
  expect(url.hostname).toEqual('127.0.0.1.com')
  expect(url.protocol).toEqual('https:')
  expect(url.host).toEqual('127.0.0.1.com')
...
it('allows to update search params', () => {
  const url = new NextURL('/example', 'http://127.0.0.1.com')
  url.searchParams.set('foo', 'bar')
  expect(url.search).toEqual('?foo=bar')
  expect(url.toString()).toEqual('http://127.0.0.1.com/example?foo=bar')
})
...
it('parses and formats the basePath', () => {
  ...
  expect(url.toString()).toEqual('http://127.0.0.1/root/example')
  ...
  url3.href = 'http://127.0.0.1.com/root/example'
  expect(url3.basePath).toEqual('/root')
  expect(url3.pathname).toEqual('/example')
  expect(url3.toString()).toEqual('http://127.0.0.1.com/root/example')
})

it('allows to get empty locale when there is no locale', () => {
  const url = new NextURL('https://127.0.0.1:3000/foo')
...
it('parses and formats the default locale', () => {
  ...
  expect(url.toString()).toEqual('http://127.0.0.1/es/bar')

  url.basePath = '/root'
  expect(url.locale).toEqual('es')
  expect(url.toString()).toEqual('http://127.0.0.1/root/es/bar')

  url.locale = 'en'
  expect(url.locale).toEqual('en')
  expect(url.toString()).toEqual('http://127.0.0.1/root/bar')

  url.locale = 'fr'
  expect(url.locale).toEqual('fr')
  expect(url.toString()).toEqual('http://127.0.0.1/root/fr/bar')
})
...
it('parses and formats the default locale with forceLocale', () => {
  ...
  url.locale = 'en'
  expect(url.locale).toEqual('en')
  expect(url.toString()).toEqual('http://127.0.0.1/root/en/bar')
...
it('consider 127.0.0.1 and variations as 127.0.0.1', () => {
  const httpUrl = new NextURL('http://127.0.0.1:3000/hello')
  ...
  const httpsUrl = new NextURL('https://127.0.0.1:3000/hello')
...
it('allows to change the port', () => {
  const url = new NextURL('https://127.0.0.1:3000/foo')
  url.port = '3001'
  expect(url.href).toEqual('https://127.0.0.1:3001/foo')
  url.port = '80'
  expect(url.href).toEqual('https://127.0.0.1:80/foo')
  url.port = ''
  expect(url.href).toEqual('https://127.0.0.1/foo')
})
...
it('allows to clone a new copy', () => {
  ...
  expect(url.toString()).toEqual('http://127.0.0.1/root/es/bar')
  expect(clone.toString()).toEqual('http://127.0.0.1/root-test/es/test')
})

it('does not add locale for api route', () => {
  const url = new NextURL('http:///127.0.0.1:3000/api', {
    nextConfig: {
      i18n: {
        defaultLocale: 'en',
        locales: ['en', 'es', 'fr'],
  ...
  let expected = 'http://127.0.0.1:3000/api'
  ...
  expected = 'http://127.0.0.1:3000/api/hello'
  expect(url.href).toEqual(expected)
  expect(url.toString()).toEqual(expected)
  expect(url.toJSON()).toEqual(expected)
})
...
it('correctly parses a prefetch url', async () => {
  ...
  expect(String(url)).toEqual(
    'http://127.0.0.1:3000/_next/data/1234/en/hello.json'
  )
})
...
it('correctly handles trailing slash in _next/data', async () => {
  ...
  expect(url.locale).toEqual('')
  expect(String(url)).toEqual('http://127.0.0.1:3000/_next/data/1234/abc.json')
})
...
it('correctly handles trailing slash in _next/data with basePath', async () => {
  ...
  expect(url.locale).toEqual('')
  expect(String(url)).toEqual(
    'http://127.0.0.1:3000/docs/_next/data/1234/abc.json'
  )
})
...
it('correctly parses a prefetch url with i18n', async () => {
  ...
  expect(url.locale).toEqual('en')
  expect(String(url)).toEqual(
    'http://127.0.0.1:3000/_next/data/development/en/hello.json'
  )
})
...
it('allows to update the pathname for a prefetch url', async () => {
  ...
  url.pathname = '/foo'
  expect(String(url)).toEqual(
    'http://127.0.0.1:3000/_next/data/development/en/foo.json'
  )
})
...
it('allows to update the pathname to the root path for a prefetch url', async () => {
  ...
  url.pathname = '/'
  expect(String(url)).toEqual(
    'http://127.0.0.1:3000/_next/data/development/index.json'
  )
})
...
it('preserves the trailingSlash', async () => {
  ...
  expect(String(url)).toEqual('http://127.0.0.1:3000/es/')
})
...
it('allows to preserve a json request', async () => {
  const url = new NextURL(
    'http://127.0.0.1:3000/_next/static/development/_devMiddlewareManifest.json',
    {
      nextConfig: {
        i18n: {
          defaultLocale: 'en',
